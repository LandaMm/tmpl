...
namespace Runtime
{
    ...
    std::shared_ptr<ComplexValueType> TypeChecker::DiagnoseFnCall(std::shared_ptr<FunctionCall> fnCall)
    {
        ...
        switch(callee->GetType())
        {
            case AST::NodeType::ObjectMember:
            {
                ...
                std::shared_ptr<ComplexValueType> targetType = DiagnoseNode(obj->GetObject());
        ...
        while (it->HasItems())
        {
            ...
            it->Next();
            std::shared_ptr<ComplexValueType> valType = DiagnoseNode(arg);
