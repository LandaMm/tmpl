...
namespace Runtime
{
    ...
    std::shared_ptr<ComplexValueType> TypeChecker::DiagnoseNode(std::shared_ptr<Node> node)
    {
        switch(node->GetType())
        {
            ...
            case NodeType::Block:
            {
                ...
                m_variables = scope;
                std::shared_ptr<ComplexValueType> value = std::make_shared<ComplexValueType>(ValueType::Null);
