...
namespace AST
{
	namespace Nodes
	{
        ...
        private:
            ...
            std::vector<std::shared_ptr<GenericNode>> m_generic_types;
        ...
        public:
            FunctionDeclaration(
                ...
                m_generic_types(std::vector<std::shared_ptr<GenericNode>>()),
                m_ret_type(nullptr),
                m_body(body),
                m_modifier(modifier),
                m_index(0),
        ...
        public:
            ...
            void AddGeneric(std::shared_ptr<GenericNode> generic);
        ...
        public: // Params
            inline std::shared_ptr<FunctionParam> GetGeneric(unsigned int index)
                { return m_params[index]; }
            inline unsigned int GetParamsSize() const { return m_params.size(); }
        public: // Generics
            inline std::shared_ptr<GenericNode> GetParam(unsigned int index)
                { return m_generic_types[index]; }
            inline unsigned int GetGenericsSize() const { return m_generic_types.size(); }
		public:
